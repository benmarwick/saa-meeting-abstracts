...
BM: Research into the history of archaeology is often organised into two major phases. An early internalist phase, where the story of development of the field is told with limited reference to the influence of political, social, religious, and economic institutions. In the 1980s an external, critical history phase began, seeking to understand the interactions between archaeological practice and its political, economic and social contexts. <!-- need to expand more on 'internalist phase' and 'critical history' phase... define and give examples... --> 

BM: We believe there is now a third phase in studying the history of archaeology... quantiative analysis of text... previous efforts at text analysis in archaeology (e.g. bibliometrics, citation networks) <!--  define and give examples... we got many of these listed in our notes already --> 
...
LW: @Kozlowski shows how word embeddings are effective for identifying semantic changes in shared understandings of US social classes in the 20th century. They used the word2vec algorithm to generate word embeddings from millions of books published between 1900 and 2012 that are available in the Google Ngrams corpus. The constructed cultural dimensions by word vector arithmetic operations on antonym pairs relating to key cultural concepts. They computed an 'affluence dimension' using arithmetic operations on word vectors, e.g. 'rich-poor', 'priceless-worthless', and 'prosperous-bankrupt', etc.. The resulting values capture a semantic sense of affluence and were used to draw a line on the plot that is the affluence dimension that other word vectors are projected on. A major finding is that employment words show substantial semantic changes on the affluence dimension over time. For example, titles of formal office, such as 'lords' and 'governor', change to 'promoter' and 'mogul', which are positions of economic status with power. Similarly, employee words shift from an association with words like 'wage' and 'earners' in the early 20th century, and 'soldier' during World War I, to 'retirement' and 'qualified' in the end of the century. This reflects the emergence of a middle-class in the US. This case study indicates word embeddings can be a robust method for examining historical trends in semantic change for words relating to social class.
...
# the highest number of abstract
...
```{r time-series, cache.extra=tools::md5sum(here::here("code/004-basic-word-stats-and-plots.R"))}
# show plot of abstract and word count
source(here::here("code/004-basic-word-stats-and-plots.R"))
knitr::include_graphics(here::here("figures/time-series-count-abstracts-and-words.png"))
```

```{r basic-word-change, cache.extra=tools::md5sum(here::here("code/005-basic-word-stats-and-plots.R"))}
# show plot of basic word change
source(here::here("code/005-basic-word-stats-and-plots.R"))
knitr::include_graphics(here::here("figures/time-series-count-abstracts-and-words.png"))
```



```{r temporal-referencing-fns, cache.extra=tools::md5sum(here::here("code/006-temp-ref-functions.R"))}
# load the functions for computing temporal referencing and drawing plots
source(here::here("code/006-temp-ref-functions.R"))
```

```{r temporal-referencing-plots, cache.extra=tools::md5sum(here::here("code/006-006-temp-ref-scripts.R"))}
# run the functions for computing temporal referencing and drawing plots
source(here::here("code/006-temp-ref-scripts.R"))
```

```{r temporal-referencing-plot-theory, cache.extra=tools::md5sum(here::here("figures/tr-plot-target-word-is-theory-5-by-year-intervals.png"))}
knitr::include_graphics(here::here("figures/tr-plot-target-word-is-theory-5-by-year-intervals.png"))
```


- basic details (e.g. word and abstract counts)

LW: Figure XX shows that there is an increase in the abstract counts over time that can be divided into two groups, a group of an early time from 1962 to 1994 with abstracts less than 1,000 per year, and a group of a later time from 2004 to 2020 with abstracts more than 1,500 per year. We observed the highest abstract counts (n = `r prettyNum(high_abs, big.mark = ",")`) in 2015, followed by 2017, 2019, 2018, and 2014 with more than 4,000 attracts, but a drop in 2020 with around 3,000 abstracts. The total number of word counts per year shows a similar pattern of growth over time that corresponds to the trend of the abstract counts. This suggests that the more abstract counts, the more word counts per year that matches our expectation for the data. <!-- more interesting to say "This suggest that the number of words per abstract has been very stable over time for the SAA meetings --> 

LW: We compared the word frequencies between two time periods, before 2004 and after 2004, to explore changes in the use of words in archaeology over time. We observed (Figure XX) an interesting pattern that shows a considerably higher proportion of words used less often, compared to words used more often after 2004, despite the word count being much higher in the later time. That is, archaeologists in more recent years have a smaller vocabulary and tend to not use some words, such as 'prehistoric', 'data', 'site', 'settlement', and 'patterns', as frequently as they did in the past. In contrast, words such as 'landscape', 'ancient', 'social', 'practices', and 'understanding' became common after 2004. It could reflect an extinction of a large group of words during the development of archaeology. One reason may be researchers have a more common idea about the selection of words when the community becomes bigger and more international. This leads to the use of a smaller group of words that present the shared understanding about archaeology instead of the use of a large group of obscure words that demonstrate one's knowledge. <!-- sounds like a value judgement --> 
