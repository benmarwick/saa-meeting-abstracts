...
LW: Another example shows how word embeddings are effective for identifying semantic changes in shared understandings of US social classes in the 20th century. @Kozlowski used the word2vec algorithm to generate word embeddings from millions of books published between between 1900 and 2012 that are available in the Google Ngrams corpus. <!-- need to prepare the reader: what are the class dimension and how were they identified? capture a sense of that they project word embeddings on a project for each cultural dimension --> The results suggest that 'employment', one of the class dimensions, shows substantial semantic changes. For example, titles of formal office, such as 'lords' and 'governor', change to 'promoter' and 'mogul', which are positions of economic status with power. Similarly, the class dimension 'employee' shifts from an association with words like 'wage' and 'earners' in the early 20th century, and 'soldier' during World War I, to 'retirement' and 'qualified' in the end of the century. This reflects the emergence of a middle-class in the US. They also found a close relationship between word embeddings in the classes, including education, morality, and cultivation, over time despite there is a major shift in the condition of economy, industry, and employment in the present day. This case study indicates the robustness of word embeddings as a method to examine historical trends by exploring social classes. <!-- check all the sentences to make sure the main ideas flow nicely through the entire paragraph -->
